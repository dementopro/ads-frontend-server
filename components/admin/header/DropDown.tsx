...
const DropDown = () => {
  ...
  const { isOpen: isStartTutorialModalOpen, onOpen: onStartTutorialModalOpen,  onClose: onStartTutorialModalClose } = useDisclosure();
  const [messageApi, contextHolder] = message.useMessage();
  const { tutorialCampaign, guideModeIndex, setIsInTutorialMode, setTutorialCampaign, setGuideModeIndex, startTutorial, getGuideModes } = useTutorialsContext();
    ...
    <>
      ...
      <div className="flex items-center">
        <span className="relative inline-flex mr-10">
          <button type="button" className="inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm shadow rounded-md text-sky-500 bg-white dark:bg-slate-800 transition ease-in-out duration-150 ring-1 ring-slate-900/10 dark:ring-slate-200/20" onClick={handleOpenTutorial}>
            Tutorial
          </button>
      ...
      <StartTutorialModal isOpen={isStartTutorialModalOpen} onClose={onStartTutorialModalClose} />
    </>
  )
}
