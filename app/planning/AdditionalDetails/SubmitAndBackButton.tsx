...
const infographics_template = {
  ...
  body: {
    ...
    values: {
      ...
      popupCloseButton_iconColor: '#23252B',
...
const SubmitAndBackButton: FC<SubmitAndBackButtonProps> = ({
  ...
  const handleSubmit = (index: number) => {
    if (isValidSeo(formData.content_type, formik.errors)) {
      ...
      bodyData.append(
        'infographics_styles',
        formData.infographics_styles.join(',')
      );
      company.assets.forEach((asset) => {
        bodyData.append('media', asset);
      });

      const getCustomizedInfographicsTemplate = (infographicsTagsObj: any) => {
          ...
          for (var key in templateObj) {
            if (typeof templateObj[key] === 'object') {
              ...
              if (key === 'elementType') {
                ...
                const infographicsTagsKeys: string[] =
                  Object.keys(infographicsTagsObj);
                const index = infographicsTagsKeys.findIndex(
                  (key: string) =>
                    key == elementType || key.startsWith(elementType)
                );
                if (index > -1) {
                  templateObj['text'] = (infographicsTagsObj as any)[
                    infographicsTagsKeys[index]
                  ];
                  delete (infographicsTagsObj as any)[
                    infographicsTagsKeys[index]
                  ];
                }
              }
        ...
        const temp = JSON.parse(JSON.stringify(customizedInfographicsTemplate));

        if (temp.body.rows[0].columns[0].contents[0].values.src) {
          temp.body.rows[0].columns[0].contents[0].values.src.url =
            infographicsTagsObj.infographicImage[0];
        }

        if (temp.body.rows[5].columns[1].contents[0].values.src) {
          temp.body.rows[5].columns[1].contents[0].values.src.url =
            infographicsTagsObj.infographicImage[1];
        }

        customizeInfographicsTemplate(temp);
        ...
        .then((res) => {
          if (res.data.status == true) {
            try {
              ...
              setInfographics(
                getCustomizedInfographicsTemplate({
                  ...infographics_obj,
      ...
      const getCustomizedInfographicsTemplate = (
        infographicsTagsObj: Object
      ) => {
        let customizedInfographicsTemplate = { ...infographics_template };
        const customizeInfographicsTemplate = (templateObj: any) => {
          for (var key in templateObj) {
        ...
        .then((res) => {
          if (res.data.status == true) {
            try {
              ...
              setLandingPage(
                getCustomizedInfographicsTemplate(landingpage_obj)
              );
            } catch {}
            setActiveButtonIndex(index);
          } else {
    ...
    <div className="flex items-center gap-10 self-stretch mt-[32px]">
      ...
      <button
        ...
        className={`${styles.submit} ${
          !isValid ? '!bg-background-300' : '!bg-primary-purple'
        }`}
        disabled={!isValid}
      >
        {formData.content_type.toLowerCase() === 'infographics'
