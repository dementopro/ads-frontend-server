...
import {
  type SocialAccountInterface,
  type SocialAccountsInterface,
  useAccountContext,
...
import fetchFacebookUserData from '@/utils/social/facebook-profile-data';
import ComingSoonPopup from '@/components/comingSoonPopup/ComingSoonPopup';

const PinterestAnalyticsModal = dynamic(
  () => import('@/app/planning/Analytics/PinterestAnalyticsModal'),
  { ssr: false }
);
const MetaAnalyticsModal = dynamic(
  () => import('@/app/planning/Analytics/MetaAnalyticsModal'),
  { ssr: false }
);
...
const SocialMediaDetails: FC<SocialMediaDetailsProps> = ({
  ...
  const [currentSocialMediaAccount, setCurrentSocialMediaAccount] =
    useState<{} | null>(null);
  const [isComingSoonPopupOpen, setIsComingSoonPopup] =
  ...
  const toggleComingSoonPopup = () => {
    setIsComingSoonPopup(!isComingSoonPopupOpen);
  };

  const oauthLogin = async () => {
    const selectedProvider: string = currentProvider;
    const providers = ['google', 'pinterest', 'facebook'];
    ...
    if (providers.includes(selectedProvider)) {
      try {
        ...
        if (providers.indexOf(selectedProvider) > -1) {
          ...
          popupCenter(
            tabsList[activeTab].page,
            `${tabsList[activeTab].title} Sign In`,
            () => {
    ...
    if (
      ...
      try {
        ...
        const accountsResponse = await axios.post(
          `/api/planning/${tabsList[activeTab].uri}`,
          {
  ...
  const fetchSocialMediaAccountData = (provider: string, sessionData: any) => {
    if (provider === 'facebook') {
      fetchFacebookUserData(
        sessionData[provider].accessToken,
        sessionData[provider].accountId
      )
        .then((response) => {
          if (response && Object.keys(response).length > 0) {
            messageApi.success(
              `You have successfully connected your Facebook account - ${response.name}`
            );
            setCurrentSocialMediaAccount(response);
          }
        })
        .catch((error) => {
          messageApi.error(
            'An error occurred while fetching data from your Facebook profile'
          );
        });
    } else if (provider === 'pinterest') {
      setCurrentSocialMediaAccount(sessionData[provider]);
      messageApi.success(
        `You have successfully connected your Pinterest account - ${sessionData[provider].accountId}`
      );
  ...
  useEffect(() => {
    ...
    if (socialAccounts[currentProvider]) {
      formikValues.url = tabsList[activeTab].url(
        socialAccounts[currentProvider].accountId
      );
    } else {
      formikValues.url = '';
    }
  ...
  useEffect(() => {
    if (
      ...
      session &&
      (session as any)['user'] &&
      (session as any)[currentProvider]
    ) {
      const sessionData: any = session;
      formik.setValues({
        ...formik.values,
        url: sessionData[currentProvider]
          ? tabsList[activeTab].url(sessionData[currentProvider].accountId)
          : '',
      });

      fetchSocialMediaAccountData(currentProvider, sessionData);
    ...
    (async () => {
      if (selectedAdAccount) {
        const analytics = await axios.post(
          `/api/planning/${
            tabsList[activeTab].uri
          }/getAnalytics?ad_account_id=${(selectedAdAccount as any).id}`,
          {
            accessToken: socialAccounts[currentProvider].accessToken,
            refreshToken: socialAccounts[currentProvider].refreshToken,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-12 gap-4 mt-8">
        <div
          id="social-media-platforms-menu"
          className={`col-span-12 !mt-0 flex items-end relative`}
        >
          {tabsList.map((tab, i) => (
            <Button
              key={`tab_${i}`}
            ...
            >
              ...
              <span className="hidden truncate md:block">{tab.title}</span>
            </Button>
          ))}
          ...
          {isInTutorialMode &&
            ...
            currentGuideMode.mode === 'OAUTH' && (
              <div className="absolute right-full flex flex-col items-end translate-x-[-10px] translate-y-[-10px]">
                <div
                  className={`w-[200px] bg-primary-purple rounded-lg text-white p-3 text-md tutorial-element mr-10 mb-2`}
                >
                  Choose which accounts you’d like to connect to for social
                  media
                </div>
                <TopToRightCurveLineArrow />
              </div>
        ...
        <div
          id="social-oauth-section"
          className={`relative ${styles.div} col-span-12 !mt-0`}
        >
          <div className="grid w-full grid-cols-12">
            <div className="col-span-12 lg:col-span-6">
              ...
              <div className="mt-6 flex flex-row">
                  ...
                  {isInTutorialMode &&
                    ...
                    currentGuideMode.mode === 'OAUTH' && (
                      <div className="absolute top-full right-full flex items-center translate-x-[-10px] translate-y-[-10px] w-fit">
                        <div
                          className={`w-[300px] bg-primary-purple rounded-lg text-white p-3 text-md tutorial-element mr-2`}
                        >
                          Click on the “{tabsList[activeTab].title}” button to
                          connect your account through 1 click authentication.
                        </div>
                        <BottomToLeftCurveLine />
                      </div>
                    )}
                ...
                <button
                  className={`flex items-center gap-2 ml-3 px-4 py-2 rounded-lg ${
                    isCurrentSocialAuthenticated
                      ? 'bg-primary-purple'
                      : 'bg-background-300'
                  } hover:brightness-110`}
      ...
      <div
        id="social-media-audience-section"
        className="grid grid-cols-12 gap-4 mt-8 relative"
      >
          ...
          currentGuideMode.mode === 'ADDITIONAL1' && (
            <Fragment>
              ...
              <div className="absolute left-[-300px] bottom-full flex items-center tutorial-element w-fit">
                <div
                  className={`w-[310px] bg-primary-purple rounded-md text-white p-3 text-md tutorial-element mr-5 mb-10`}
                >
                  Upload any media assets that you’d like to optimize and watch
                  as our AI generates targeted media based on what you upload
                </div>
      ...
      <div
        id="social-customer-schedule-section"
        className="relative grid grid-cols-12 gap-4 mt-8"
      >
        <div className={`${styles.div} col-span-12 lg:col-span-6 !mt-0`}>
          <p className=" text-[15px] text-white not-italic font-medium leading-[normal]">
        ...
        <div className={`${styles.div} col-span-12 lg:col-span-6 !mt-0`}>
          ...
          <EmailScheduleModal
            ...
            title="Ad Schedule"
            description="Select the days & times you would like to schedule your ads"
          />
        </div>
            ...
            <Fragment>
              ...
              <div className="absolute right-[50px] bottom-full flex items-center tutorial-element">
                ...
                <div
                  className={`w-[310px] bg-primary-purple rounded-md text-white p-3 text-md tutorial-element ml-10 mb-20`}
                >
                  Click on “ad schedule” to enter preferred time and dates for
                  sending out ads.
                  <br />
                  <br />
        ...
        {isComingSoonPopupOpen && (
          <ComingSoonPopup
            ...
            togglePopup={toggleComingSoonPopup}
          />
        )}
      </div>
