...
const options = ['Minimalist', 'Creative', 'Business', 'Technology'];

interface InfographicsDetailsProps {
  formData: CompanyDetailForm;
  formik: ReturnType<typeof useFormik<CompanyForm>>;
  setFormData: (formData: CompanyDetailForm) => void;
}

const InfographicsDetails: FC<InfographicsDetailsProps> = ({
  formData,
  formik,
  setFormData,
}) => {
                  ...
                  onClick={() => {
                    if (formData.infographics_styles.includes(val)) {
                      setFormData({
                        ...formData,
                        infographics_styles:
                          formData.infographics_styles.filter(
                            (item) => item != val
                          ),
                      });
                    } else {
