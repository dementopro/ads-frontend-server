...
import { BiChevronLeft, BiExpand, BiSave, BiDownload } from 'react-icons/bi';
...
const InfographicsRecommendation = () => {
  ...
  const handleEditorFullScreen = () => {
    const element: any = document.getElementById('infographics-editor');
  ...
  return (
    <div id="infographics-editor" className={`${styles.onPageDiv} overflow-x-auto`}>
      {contextHolder}
      ...
      <StyledEditorWrapper className={`${styles.mainDiv} w-full h-full !p-0`}>
        ...
        {
          infographicsMode === 'EDIT' && <EmailEditor options={{ className: "w-full" }} onLoad={onLoad} onReady={onReady} />
        }
      </StyledEditorWrapper>
    </div>
  );
