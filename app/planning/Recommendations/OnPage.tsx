...
import { TopToRightBezierCurveLineArrow, BottomToRightCurveLineArrow, LongTopToLeftBezierCurveLineArrow } from '@/components/tutorial/Arrows';
...
const OnPage = () => {
  ...
  return (
    <div className={`${styles.onPageDiv} ${isInTutorialMode && tutorialCampaign === 'SEO' && currentGuideMode.mode === 'RECOMMENDATION' ? 'overflow-visible' : 'overflow-x-auto'}`}>
      { onpage.map((issue, i) => (
        <div id={i === 0 ? 'first-seo-on-result': ''} className={`${styles.mainDiv} gap-y-[24px] w-full relative`} key={`onpage_issue_${i}`}>
          ...
          <div className='block w-full'>
            <div className="flex items-center justify-between gap-4">
              ...
              <button className="px-5 py-3 not-italic font-semibold leading-5 text-center text-white rounded-lg bg-primary-purple relative" onClick={() => {
                router.push(`/contentType/seo?type=0&url=${issue.url}`);
              }}>
                <span className="text-[13.5px]">View</span>

                {
                  isInTutorialMode && tutorialCampaign === 'SEO' && currentGuideMode.mode === 'RECOMMENDATION' && i === 0 && (
                    <div className="absolute right-0 top-full translate-x-[-20px] translate-y-[10px] flex justify-end tutorial-element">
                      ...
                      <div className="absolute right-0 top-full translate-y-[10px]">
                        <div className={'!w-[310px] bg-primary-purple rounded-xl text-white p-5 text-md'}>
                          Click “view” to review specific optimizations for search engine friendliness
                        </div>
                      </div>
                    </div>
                  )
            ...
            isInTutorialMode && tutorialCampaign === 'SEO' && currentGuideMode.mode === 'RECOMMENDATION' && i === 0 && (
              <Fragment>
                ...
                <div className="absolute left-[120px] bottom-full translate-y-[-90px] flex items-center tutorial-element">
                  ...
                  <div className={`w-[310px] bg-primary-purple rounded-md text-white p-3 text-md tutorial-element ml-5 mb-[200px]`}>
                ...
                <div className="absolute left-0 top-full w-full translate-y-[5px] flex tutorial-element">
                  <div className={'!w-[310px] bg-primary-purple rounded-xl text-white p-5 text-md translate-x-[-100px] translate-y-[30px] mr-5'}>
                    🎉Congratulations!🎉 You now have tailored recommendations to help improve your SEO
                  </div>
